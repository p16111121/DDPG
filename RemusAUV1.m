function [Mmat,Props,X,Y,Z,K,M,N] = RemusAUV1()
...
D = 1.7e-1; % m
L = 1.8057; % m
Dratio=(1.7e-1/1.91e-1);
Lratio=(1.8057/ 1.33);
g = 9.81;
rho = 1030;% kg/m^3
m = 180/9.81; % kg
W = 180; % N
B = 180; % N  %(1 + 1e-7, 1 + 1e-8)
Af = (2.85e-2)*(1.7e-1/1.91e-1)*(1.7e-1/1.91e-1);
Sw = (7.09e-1)*(1.7e-1/1.91e-1)*(1.8057/ 1.33);
Ap = (2.26e-1)*(1.7e-1/1.91e-1)*(1.8057/ 1.33);
V = (3.15e-2)*(Dratio^2)*Lratio;

Ixx = (1.77e-1)*(1.7e-1/1.91e-1)*(1.7e-1/1.91e-1); Ixy = 0; Ixz = 0; %kg*m^2
Iyy = 3.45*(1.7e-1/1.91e-1)*(1.8057/ 1.33); Iyx = Ixy; Iyz = 0;
Izz = 3.45*(1.7e-1/1.91e-1)*(1.8057/ 1.33); Izx = Ixz; Izy = Iyz;
% Af = 2.85e-2;
% Sw = 7.09e-1;
% Ap = 2.26e-1;
% V = 3.15e-2;
% 
% Ixx = 1.77e-1; Ixy = 0; Ixz = 0; %kg*m^2
% Iyy = 3.45; Iyx = Ixy; Iyz = 0;
% Izz = 3.45; Izx = Ixz; Izy = Iyz;

xB = 0; yB = 0; zB = 0;
xG = 0; yG = 0; zG = 0.03; %1.96e-2
